/**
 * [TC-ADV-P06] 자문 승인 — 성능 측정
 *
 * ENV: BASE_URL, LOGIN_EMAIL, LOGIN_PASSWORD, ADVICE_ID(optional)
 */
...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-ADV-P06_detail.png` });
    await approveAdvice(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-ADV-P06_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-ADV-P06_summary_${timestamp}.html`]: htmlReport(data) };
}
