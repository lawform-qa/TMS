/**
 * [TC-ADV-P01] 자문 임시저장 목록 조회 — 성능 측정
 *
...
export default async function () {
    ...
    await page.screenshot({ path: `screenshots/${ts()}_TC-ADV-P01_draft_list.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-ADV-P01_summary_${timestamp}.html`]: htmlReport(data) };
}
