...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-ADV-P04_detail.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-ADV-P04_summary_${timestamp}.html`]: htmlReport(data) };
}
