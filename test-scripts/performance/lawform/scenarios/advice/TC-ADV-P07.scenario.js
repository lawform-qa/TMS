/**
 * [TC-ADV-P07] 자문 반려 — 성능 측정
 *
 * ENV: BASE_URL, LOGIN_EMAIL, LOGIN_PASSWORD, ADVICE_ID(optional), DENY_REASON(optional)
 */
...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-ADV-P07_detail.png` });
    await denyAdvice(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-ADV-P07_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-ADV-P07_summary_${timestamp}.html`]: htmlReport(data) };
}
