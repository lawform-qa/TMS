...
import { denyFinalReview } from '../../actions/clm/clm.final.js';
...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P10_detail.png` });
    await denyFinalReview(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P10_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-CLM-P10_summary_${timestamp}.html`]: htmlReport(data) };
}
