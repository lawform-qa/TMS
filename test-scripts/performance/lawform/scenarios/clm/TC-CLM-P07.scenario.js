...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P07_detail.png` });
    await approveFinancialReview(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P07_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-CLM-P07_summary_${timestamp}.html`]: htmlReport(data) };
}
