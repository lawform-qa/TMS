/**
 * [TC-CLM-P12] CLM 인감 승인 — 성능 측정
 *
 * ENV: BASE_URL, LOGIN_EMAIL, LOGIN_PASSWORD, SEAL_ID(optional)
 */
...
export default async function () {
    ...
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P12_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-CLM-P12_summary_${timestamp}.html`]: htmlReport(data) };
}
