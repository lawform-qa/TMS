...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P05_detail.png` });
    await denyLegalReview(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P05_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-CLM-P05_summary_${timestamp}.html`]: htmlReport(data) };
}
