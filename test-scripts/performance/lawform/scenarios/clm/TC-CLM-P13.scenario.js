/**
 * [TC-CLM-P13] CLM 인감 반려 — 성능 측정
 *
 * ENV: BASE_URL, LOGIN_EMAIL, LOGIN_PASSWORD, SEAL_ID(optional), DENY_REASON(optional)
 */
...
import { gotoDetailOrFirst } from '../../actions/seal/seal.navigate.js';
...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P13_detail.png` });
    await denySeal(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P13_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-CLM-P13_summary_${timestamp}.html`]: htmlReport(data) };
}
