...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P06_detail.png` });
    await requestFinancialReview(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P06_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-CLM-P06_summary_${timestamp}.html`]: htmlReport(data) };
}
