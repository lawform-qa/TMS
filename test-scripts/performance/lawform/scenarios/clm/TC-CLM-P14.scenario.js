...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P14_detail.png` });
    await requestEsign(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P14_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-CLM-P14_summary_${timestamp}.html`]: htmlReport(data) };
}
