...
export default async function () {
    ...
    if (!entered) return;
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P04_detail.png` });
    await approveLegalReview(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-CLM-P04_done.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-CLM-P04_summary_${timestamp}.html`]: htmlReport(data) };
}
