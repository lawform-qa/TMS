...
import { gotoReviewList } from '../../actions/seal/seal.navigate.js';
...
export default async function () {
    ...
    await page.screenshot({ path: `screenshots/${ts()}_TC-SEAL-P01_list.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-SEAL-P01_summary_${timestamp}.html`]: htmlReport(data) };
}
