...
import { clickNewSealButton } from '../../actions/seal/seal.draft.js';
...
export default async function () {
    ...
    await page.screenshot({ path: `screenshots/${ts()}_TC-SEAL-P02_before.png` });
    await clickNewSealButton(page);
    await page.screenshot({ path: `screenshots/${ts()}_TC-SEAL-P02_after.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-SEAL-P02_summary_${timestamp}.html`]: htmlReport(data) };
}
