...
export default async function () {
    ...
    await page.screenshot({ path: `screenshots/${ts()}_TC-MISC-P03_setup.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-MISC-P03_summary_${timestamp}.html`]: htmlReport(data) };
}
