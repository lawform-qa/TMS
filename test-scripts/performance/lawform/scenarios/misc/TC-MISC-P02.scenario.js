...
export default async function () {
    ...
    await page.screenshot({ path: `screenshots/${ts()}_TC-MISC-P02_bulk.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-MISC-P02_summary_${timestamp}.html`]: htmlReport(data) };
}
