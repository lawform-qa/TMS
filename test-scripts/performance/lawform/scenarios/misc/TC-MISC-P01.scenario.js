...
export default async function () {
    ...
    await page.screenshot({ path: `screenshots/${ts()}_TC-MISC-P01_dashboard.png` });
}

export function handleSummary(data) {
    const timestamp = getFormattedTimestamp().replace(/:/g, '_');
    return { [`Result/TC-MISC-P01_summary_${timestamp}.html`]: htmlReport(data) };
}
