...
export default async function clm_draftStop(page, options = {}) {
  ...
  try {
    await login_to_dashborad(page);
      ...
      // 계약 검토 요청 작성 시작
      if (options.CONTRAT_UPLOAD === "use") { // 계약서 업로드 여부 : 사용
        ...
        // 편집기 사용 여부
        if (options.EDITER_USE === "use") { // 편집기 사용 여부 : 사용
        ...
        // 계약서 첨부 방식
        if (options.CONTRAT_TYPE === "file") { // 계약서 첨부 방식 : 파일 업로드
        ...
        // 계약서 선택
        if (options.CONTRAT_SELECT === "file") { // 계약서 첨부 방식 : 파일 업로드
        ...
        // 보안 여부
        if (options.SECURITY_TYPE === "all") { // 보안 여부 : 전체 공개
        ...
        else if (options.SECURITY_TYPE === "refer") { // 보안 여부 : 참조인
        ...
        // 검토 진행 여부
        if (options.REVIEW_TYPE === "use") { //검토 진행 여부 : 검토 필요
        ...
        // 검토 마감 기한
        // 계약 검토 요청
        if (options.APPROVAL_SET === "use") { // 내부 결재선이 있는 경우
      ...
      console.error('clm_draft.stop.js 실행 중 오류:', error);
      throw error;
    }
}